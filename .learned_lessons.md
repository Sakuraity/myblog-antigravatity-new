...
- [2025-12-19] **即刻动态添加 SOP**: 用户指令为“添加notes，[即刻链接]”时，遵循以下 SOP：1. 使用 `browser_subagent` 打开链接；2. **智能等待**：先检查内容是否可见，若可见则直接提取；若不可见（显示登录页），则等待 20s 让用户授权登录；3. 提取文字和图片；4. 创建笔记文件，`tags` 设为 `["Jike", "随想"]`，添加原文链接按钮。
