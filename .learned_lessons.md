...
- [2026-01-21] **批量 URL 工作流的安全执行策略**: 当用户一次性提供多个 URL 需要执行相同 Workflow 时，按顺序逐个处理（Sequential execution）比并行处理更安全稳定，尤其是涉及 Browser Subagent 资源时。同时，为每个 URL 自动生成独立且有意义的文件名（Slug）对于笔记系统的整洁非常重要。[Context: Batch processing Jike notes]

- [2026-01-21] **Astro Content Schema 陷阱**: 对于定义为 `z.number().min(1).optional()` 的字段（如评分），提供 `0` 值会直接导致构建验证失败（Check pass, Build fail）。对于尚未达标（如未评分）的条目，必须完全移除该字段而非设为 0。 [Context: Library rating schema]

- [2026-01-21] **YAML Frontmatter 引号转义**: 在 Frontmatter 中使用双引号包裹字符串时，如果字符串内部本身包含中文引号 `"..."` 或英文双引号，会导致 YAML 解析失败（无错误信息，静默 exit code 1）。解决方案：对于可能包含引号的长文本字段（如书籍简介 `comment`），应使用**单引号**包裹外层字符串。[Context: Douban book description]
